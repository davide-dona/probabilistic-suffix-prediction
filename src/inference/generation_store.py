...
# One run's duration of each of its activities. Timestamps are these accumulated, so they are
# not written a second time.
_ACTIVITY_DURATIONS = pa.list_(pa.field(name='element', type=pa.float64()))
...
_SCHEMA = pa.schema(
    [
        ...
        (
            'generated_activity_duration_minutes',
            pa.list_(pa.field(name='element', type=_ACTIVITY_DURATIONS)),
        ),
        ...
        ('point_activity_duration_minutes', _ACTIVITY_DURATIONS),
        ('point_remaining_time_minutes', pa.float64()),
        ('true_activities', _ACTIVITIES),
        ('true_activity_duration_minutes', _ACTIVITY_DURATIONS),
    ...
    rows = [
        {
            ...
            'generated_activity_duration_minutes': [
                sample.activity_duration_minutes for sample in generation.samples
            ],
            ...
            'point_activity_duration_minutes': generation.point.activity_duration_minutes,
            'point_remaining_time_minutes': generation.point.remaining_time_minutes,
            'true_activities': generation.truth.activities,
            'true_activity_duration_minutes': generation.truth.activity_duration_minutes,
        ...
        generations.append(
            Generation(
                ...
                samples=[
                    ...
                    for activities, activity_duration_minutes, remaining_time_minutes in zip(
                        columns['generated_activities'][position],
                        columns['generated_activity_duration_minutes'][position],
                ...
                point=DecodedEvents(
                    ...
                    activity_duration_minutes=columns['point_activity_duration_minutes'][position],
                    remaining_time_minutes=columns['point_remaining_time_minutes'][position],
                ),
                truth=DecodedEvents(
                    activities=columns['true_activities'][position],
                    activity_duration_minutes=columns['true_activity_duration_minutes'][position],
