...
class Loss(ScalarMetrics):
    ...
    One shape for every architecture, so shared terms read the same whichever produced them.
    Each model computes it in `SuffixModel.compute_loss` and leaves terms it does not use at 0.0.
    """

    loss: float = 0.0
    reconstruction_loss: float = 0.0
    # Separate upstream length prediction used by fixed-length generators.
